(function () {
    ...
    if (!review) return;
    const editable = review.dataset.draftStatus !== 'APPLIED';
    let dragged = null;
    ...
    const draggableItems = Array.from(review.querySelectorAll('.draft-section[data-move-url], .plan-elements > .plan-element[data-move-url]'));

    const updateDraggableState = () => {
        const isMobile = isMobileDevice();
        draggableItems.forEach(item => {
            item.draggable = !isMobile && editable;
    ...
    sections?.addEventListener('dragover', event => {
        if (isMobileDevice() || !dragged?.classList.contains('draft-section')) return;
        event.preventDefault();
