...
class AiWizardCompletionServiceTest {
    ...
    @Mock
    private AiWorkflowCompletionTokenRepository completionTokenRepository;

    @Mock
    private AiWorkflowInitializationService persistenceService;
    ...
    void parallelCompletionResolvesUniqueConflictToExistingWorkflow() throws Exception {
        ...
        when(completionTokenRepository.findOwnedWorkflowIdByToken(token, userId)).thenAnswer(invocation ->
                lookups.incrementAndGet() <= 2
                        ? Optional.empty()
                        : Optional.of(existingWorkflow.getId()));
        when(workflowRepository.findById(expected.workflowId())).thenReturn(Optional.of(existingWorkflow));
        ...
        AiWizardCompletionService service = new AiWizardCompletionService(
                workflowRepository, completionTokenRepository, persistenceService);
    ...
    void missingSessionSnapshotAfterParallelCompletionResolvesToExistingWorkflow() {
        ...
        AiWorkflowCompletion expected = new AiWorkflowCompletion(UUID.randomUUID(), UUID.randomUUID());
        AiPlanGenerationWorkflow existingWorkflow = existingWorkflow(expected);
        UUID existingWorkflowId = existingWorkflow.getId();
        when(completionTokenRepository.findOwnedWorkflowIdByToken(token, userId))
                .thenReturn(Optional.empty())
                .thenReturn(Optional.of(existingWorkflowId));
        when(workflowRepository.findById(expected.workflowId())).thenReturn(Optional.of(existingWorkflow));
        AiWizardCompletionService service = new AiWizardCompletionService(
                workflowRepository, completionTokenRepository, persistenceService);
