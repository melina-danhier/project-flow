...
class GenerationResponseValidatorTest {
    ...
    void rejectsMissingPhasesAndTasks() {
        ...
        assertCodes(validator.validate(plan(phase("phase-1", 1, PROJECT_START, PROJECT_END,
                List.of(), List.of())), scheduledRequest()), "TASK_MISSING");
    }
    ...
    void rejectsDuplicateTemporaryIds() {
        GeneratedPhase phase = phase("same-id", 1, PROJECT_START, PROJECT_END,
                List.of(task("same-id", "Aufgabe", 1, PROJECT_START, PROJECT_START)), List.of());
        assertCodes(validator.validate(plan(phase), scheduledRequest()), "TEMP_ID_DUPLICATE");
    }
    ...
    @Test
    void rejectsMixedDatedAndUndatedPlanning() {
        GeneratedPhase scheduledWithUndatedTask = phase("phase-1", 1, PROJECT_START, PROJECT_END,
                List.of(task("task-1", "Aufgabe", 1, null, null)),
                List.of(milestone("milestone-1", 1, PROJECT_END)));
        assertCodes(validator.validate(plan(scheduledWithUndatedTask), scheduledRequest()),
                "DATE_PLANNING_INCONSISTENT");

        GeneratedPhase undatedWithScheduledTask = phase("phase-1", 1, null, null,
                List.of(task("task-1", "Aufgabe", 1, PROJECT_START, PROJECT_START)), List.of());
        assertCodes(validator.validate(plan(undatedWithScheduledTask), undatedRequest()),
                "DATE_PLANNING_INCONSISTENT");
    }
    ...
    void acceptsFullyValidDatedAndUndatedPlans() {
        ...
        GeneratedPhase undated = phase("phase-1", 1, null, null,
                List.of(task("task-1", "Aufgabe", 1, null, null)), List.of());
    ...
    private GeneratedPlanResponse validDatedPlan() {
        return plan(phase("phase-1", 1, PROJECT_START, PROJECT_END,
                List.of(task("task-1", "Aufgabe", 1, PROJECT_START, PROJECT_END)),
                List.of(milestone("milestone-1", 1, PROJECT_END))));
    }
